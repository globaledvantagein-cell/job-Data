...
let currentIndex = 0;
const deadKeys = new Set(); // keys that returned 400 (invalid) — skip them
...
function getNextKey() {
    ...
    // Find a live key, skipping dead ones
    ...
    throw new Error('[ResumeMatch] All API keys are dead (400). Replace keys in .env and restart.');
}

function markKeyDead(index) {
    deadKeys.add(index);
    const alive = GEMINI_API_KEYS.length - deadKeys.size;
    console.warn(`[ResumeMatch] Key #${index} marked dead (400). ${alive} key(s) remaining.`);
}
...
async function executeWithRetries(body, label) {
    ...
    while (true) {
        ...
        try {
            ...
            // Invalid API key — mark it dead so we never use it again.
            if (status === 400) {
                markKeyDead(index);
