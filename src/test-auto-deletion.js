...
//
// Run: node src/test-auto-deletion.js
...
async function run() {
    ...
    // ── Run the deletion logic (copied from databaseManager.js) ───────────
    // We replicate the exact same queries here to test against the test collection.
    // This ensures the test matches the production code exactly.
    ...
    if (failed > 0) {
        console.log(`⚠️  ${failed} test(s) failed. Check deleteOldJobs() in databaseManager.js`);
        await client.close();
        process.exit(1);
    } else {
        ...
        console.log('   Delete this file: rm src/test-auto-deletion.js');
        await client.close();
        process.exit(0);
    }
}
